...
function fpToggleCreatePanel(open) {
  const panel = document.getElementById('fpCreatePanel');
  const trigger = document.getElementById('fpOpenCreateBtn');
  if (!panel) return;
  const shouldOpen = open == null ? panel.hidden : !!open;
  panel.hidden = !shouldOpen;
  if (trigger) trigger.hidden = shouldOpen;
  if (!shouldOpen) fpResetPickers();
}
...
function fpSelectPlayer(n, name) {
  ...
  // The empty pair row saves itself as soon as both sides are selected.
  if (fpSelected[1] && fpSelected[2]) {
    setTimeout(function() { modifyFixedPair(); }, 0);
  }
}
...
function hideImportModal() {
  const modal = document.getElementById('newImportModal');
  if (modal) modal.style.display = 'none';
}

// Register any newly imported club player automatically before adding them to the session.
// The club name is used as the player's default claim password, matching the existing
// pre-registration / claim-account flow. Existing club memberships are left untouched.
async function autoRegisterImportedClubPlayers(importPlayers) {
  const club = (typeof getMyClub === 'function') ? getMyClub() : null;
  if (!club || !club.id || !Array.isArray(importPlayers) || !importPlayers.length) return;

  const clubName = String(club.name || localStorage.getItem('kbrr_my_club_name') || '').trim();
  if (!clubName) throw new Error('Club name is required to register new players.');

  let memberships = await sbGet('memberships',
    'club_id=eq.' + encodeURIComponent(club.id) + '&select=nickname').catch(function() { return []; });
  const existing = new Set((memberships || []).map(function(m) {
    return String(m.nickname || '').trim().toLowerCase();
  }).filter(Boolean));

  for (const p of importPlayers) {
    let name = String(p.displayName || p.name || '').trim();
    if (!name) continue;
    if (typeof scsRequireValidPlayerName === 'function') name = scsRequireValidPlayerName(name);
    ...
    try {
      ...
      if (!created || !created[0] || !created[0].id) throw new Error('Player registration failed.');
      ...
      // A concurrent import may have created the same membership. Re-check before failing.
      const nowExists = await sbGet('memberships',
        'club_id=eq.' + encodeURIComponent(club.id) +
        '&nickname=ilike.' + encodeURIComponent(name) + '&select=id&limit=1').catch(function() { return []; });
      if (nowExists && nowExists.length) {
        existing.add(key);
        continue;
      }
...
async function addPlayersFromInputUI(replace) {
  const importPlayers = (newImportState.selectedPlayers || []).map(function(p) {
    return Object.assign({}, p);
  });
  if (!importPlayers || importPlayers.length === 0) { alert('No players selected!'); return; }

  try {
    await autoRegisterImportedClubPlayers(importPlayers);
  } catch (e) {
  ...
  const modal = document.getElementById('newImportModal');
  const isVaultSlotReturn = window._newImportReturnMode === 'vaultSlot' ||
    !!window._vsAddPlayersTargetSlotId ||
    !!(modal && modal.dataset && (modal.dataset.returnMode === 'vaultSlot' || modal.dataset.targetSlotId));

  if (isVaultSlotReturn) {
    ...
    if (typeof vaultSlotsReceiveImportedPlayers === 'function') {
      vaultSlotsReceiveImportedPlayers(importPlayers).catch(function(e) {
  ...
  // Build selected name set for quick lookup
  const selectedKeys = new Set(importPlayers.map(p => p.displayName.trim().toLowerCase()));

  // Remove players no longer in selected list (they were deselected in the modal)
  const removedNames = [];
  for (let i = schedulerState.allPlayers.length - 1; i >= 0; i--) {
    const key = schedulerState.allPlayers[i].name.trim().toLowerCase();
    if (!selectedKeys.has(key)) {
      removedNames.push(schedulerState.allPlayers[i].name);
      schedulerState.allPlayers.splice(i, 1);
    }
  }
  // Release DB slots for removed players
  if (removedNames.length > 0 && typeof dbReleaseSessionSlots === 'function') {
    dbReleaseSessionSlots(removedNames);
  }

  // Add newly selected players not already in list
...
function addPlayersFromText() {
  ...
  for (let i = startIndex; i < stopIndex; i++) {
    ...
    const pm = line.match(/\(([^)]+)\)/);
    if (pm) {
      const insideRaw = pm[1].trim();
      const inside = insideRaw.toLowerCase();
      if (genderLookup[inside]) {
        gender = genderLookup[inside];
        line = line.replace(/\([^)]+\)/, "").trim();
      } else if (insideRaw) {
        // SlotBookings-style names use parentheses for the name to use in play,
        // e.g. "Krishna (Kalyan)" -> "Kalyan".
        line = insideRaw;
      }
    }
    // Remove decorative emoji/symbols while preserving letters from all languages.
    // e.g. "Shivam✨" -> "Shivam".
    try {
    ...
    line = line.replace(/[\uFE0E\uFE0F]/g, "").replace(/\s{2,}/g, " ").trim();
    if (!line) continue;
...
function updatePlayerList() {
  ...
  // Sync MBM waiting queue if session active
  if (typeof mbmSyncPlayers === 'function') mbmSyncPlayers();
