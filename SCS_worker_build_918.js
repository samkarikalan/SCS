...
function parseOfflineLibrarySignature(signature) {
  ...
  const spec = {
    engineVersion: parts[0], gameType: parts[1], courtCount: 0, playerCount: 0,
    roundCount: 0, algorithm: "standard", randomOrder: true, uniquePairMode: true,
    fixedPairCount: 0, menCount: 0, womenCount: 0, topRatedCount: 0, bottomRatedCount: 0
  };
  for (const part of parts.slice(2)) {
    let m;
    if ((m = /^c(\d+)$/.exec(part))) spec.courtCount = Number(m[1]);
    else if ((m = /^p(\d+)$/.exec(part))) spec.playerCount = Number(m[1]);
    else if ((m = /^m(\d+)f(\d+)$/.exec(part))) { spec.menCount = Number(m[1]); spec.womenCount = Number(m[2]); }
    else if ((m = /^top(\d+)bottom(\d+)$/.exec(part))) { spec.topRatedCount = Number(m[1]); spec.bottomRatedCount = Number(m[2]); }
...
async function handleOfflineLibraryManifest(request, env) {
  ...
  try { await requireOfflineLibrarySession(body, env); }
...
function validateOfflineLibraryPayload(payload, spec) {
  ...
  const validName = /* @__PURE__ */ __name((name) => {
...
function selectRestingAndPlaying(state) {
  ...
  if (fullyGenderConstrained && !fixedPairs.length) {
    const genderOf = /* @__PURE__ */ __name((player) => String(getGender(player, allPlayers) || "").toLowerCase(), "genderOf");
  ...
  if (hasTypedCourts && numResting > 0) {
    let swapIn = function(needed, currentPlaying, restPool, surplusPlaying) {
    ...
    __name(swapIn, "swapIn");
    const genderOf = /* @__PURE__ */ __name((p) => {
...
function reorderFreePlayersByLastRound(freePlayers, lastRound, numCourts) {
  ...
  let c = 0;
  const distribute = /* @__PURE__ */ __name((list) => {
    for (const p of list) {
...
function getGameTierRule(pair1, pair2, tierMap) {
  const sig = /* @__PURE__ */ __name((pair) => [...pair].map((p) => tierMap[p] || "inter").sort().join("+"), "sig");
...
function sharedRoundStrict(state, useBalancedBands, standardPath = "") {
  ...
  const rating = /* @__PURE__ */ __name((name) => getRating(name, allPlayers), "rating");
  const gender = /* @__PURE__ */ __name((name) => String(getGender(name, allPlayers) || "").toLowerCase(), "gender");
  ...
  function standardMixedResting() {
    ...
    if (types.length === 3 && types.filter((type) => type === "md").length === 1 && types.filter((type) => type === "ld" || type === "wd").length === 1 && types.filter((type) => type === "xd").length === 1 && allocation.menRest === 2 && allocation.womenRest === 1) {
      ...
      const spread = /* @__PURE__ */ __name((pool, history, selected2) => {
  ...
  const compareVector = /* @__PURE__ */ __name((a, b) => {
  ...
  const pkey = /* @__PURE__ */ __name((a, b) => [a, b].sort().join("&"), "pkey");
  ...
  const singlesTypeKey = /* @__PURE__ */ __name((type) => ["md", "men", "singles-men"].includes(type) ? "singles-men" : ["ld", "wd", "women", "ladies", "singles-women"].includes(type) ? "singles-women" : "singles-free", "singlesTypeKey");
  ...
  function solve(playing) {
    ...
    const playingSet = new Set(playing);
    if (fastStandardFree) {
      let bounded = function(position, available, games, score) {
        if (++nodes2 > 2500) return;
        ...
        for (const candidate of candidates) {
          ...
          if (returningOnCourt > maxReturningPerCourt) continue;
          const next = new Set(available);
          candidate.players.forEach((name) => next.delete(name));
          bounded(position + 1, next, [...games, candidate], score.map((value, index) => value + (candidate.score[index] || 0)));
      ...
      for (const group of combinations(ordered, 4)) {
        for (const [pair1, pair2] of [
          ...
          candidates.push({ pair1, pair2, players: group, score: [pairRepeats, opponentRepeats] });
        }
      }
      candidates.sort((a, b) => compareVector(a.score, b.score));
      let best2 = null, nodes2 = 0;
      bounded(0, new Set(ordered), [], [0, 0]);
    ...
    if (fastStandardXD) {
      let boundedXd = function(position, available, games, score) {
    ...
    if (fastStandardMixed && types.length === 3 && types.filter((type) => type === "md").length === 1 && types.filter((type) => type === "ld" || type === "wd").length === 1 && types.filter((type) => type === "xd").length === 1) {
      ...
      const projectedSpread = /* @__PURE__ */ __name((pool, history, selected) => {
      ...
      const bestSameGenderGame = /* @__PURE__ */ __name((group) => {
        const arrangements = [
          [[group[0], group[1]], [group[2], group[3]]],
          [[group[0], group[2]], [group[1], group[3]]],
          [[group[0], group[3]], [group[1], group[2]]]
...
function standardSinglesRound(state) {
  ...
  const gender = /* @__PURE__ */ __name((name) => String(getGender(name, allPlayers) || "").toLowerCase(), "gender");
  const eligible = /* @__PURE__ */ __name((name, type) => {
  ...
  const choose = /* @__PURE__ */ __name((items, needed, start = 0, picked = [], output = []) => {
  ...
  const queuePosition = new Map(queue.map((name, index) => [name, index]));
  const typeKey = /* @__PURE__ */ __name((type) => ["md", "men", "singles-men"].includes(type) ? "singles-men" : ["ld", "wd", "women", "ladies", "singles-women"].includes(type) ? "singles-women" : "singles-free", "typeKey");
  ...
  const compare = /* @__PURE__ */ __name((left, right) => {
  ...
  for (const resting of restOptions) {
    let search = function(position, available, games) {
      if (position === order.length) {
        const typeSpreads = [];
